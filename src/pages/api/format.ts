...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // DOCX comes as HTML — perfect heading detection, no AI needed
    const markdown = mode === 'html'
      ? htmlToMarkdown(content)
      : smartSplitText(content);   // PDF/TXT — smart splitter
...
// ─────────────────────────────────────────────────────────────
// SMART TEXT SPLITTER
// Handles PDFs where all text is concatenated into one block
// ─────────────────────────────────────────────────────────────
function smartSplitText(raw: string): string {

  // Step 1 — normalise whitespace but keep newlines
  let text = raw.replace(/\r\n/g, '\n').replace(/\r/g, '\n');

  // Step 2 — If the text is mostly one giant paragraph (few newlines),
  // insert newlines before likely headings using regex patterns.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // ── Bullet lines ──
    if (/^[•\-\*]\s/.test(line)) {
      if (inTable) { flushTable(tableBuffer, out); tableBuffer = []; inTable = false; }
      out.push('- ' + line.replace(/^[•\-\*]\s*/, '').trim());
      continue;
    }

    // ── Table detection (2+ space-separated values per line) ──
...
// Split concatenated text by inserting newlines before headings
function splitConcatenatedText(text: string): string {

  // Known heading trigger patterns — common in blog/report PDFs
  ...
  // Split on bullet separator "• " that appears mid-sentence
  result = result.replace(/\s*•\s*/g, '\n• ');

  return result;
}

function detectHeadingLevel(line: string, next: string): string | null {
  ...
  // Must have mostly capitalised or title-case words
  const capitalWords = words.filter(w => /^[A-Z]/.test(w)).length;
  if (capitalWords / wordCount < 0.5) return null;

  // Ends without sentence punctuation (strong heading signal)
  const noEndPunct = !/[.!?,;]$/.test(trimmed);
  // Next line is longer (body text follows)
  const nextIsBody = next.length > trimmed.length;
  // Ends with question mark (definitely a heading)
...
function splitTableRow(line: string): string[] {
  // Split on 2+ spaces or tabs (common in PDF table extraction)
  return line.split(/\s{2,}|\t/).map(c => c.trim()).filter(c => c.length > 0);
}

function flushTable(buffer: string[], out: string[]) {
  if (buffer.length > 0) {
    out.push('');
    out.push(...buffer);
    out.push('');
...
// ─────────────────────────────────────────────────────────────
// HTML → MARKDOWN (for Word .docx files via mammoth)
// ─────────────────────────────────────────────────────────────
function htmlToMarkdown(html: string): string {
  ...
  // Convert headings, inline formatting, paragraphs
  processed = processed
...
function detectCategory(md: string): string {
